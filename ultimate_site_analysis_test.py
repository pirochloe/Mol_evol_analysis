...
with open("site_analysis_output_2.txt","w") as g:
    ...
    for SSS in s:
        g.write(str(SSS)+"\n")
        g.write("super_conservative_site:"+str(A[str(SSS)])+"\n")
...
if focus_group=={}:
    ...
    plt.grid()
    fig.savefig("histogram_super"+str(focus_group)+".png")
    D=[]
    D_2=[]
    
    for spell_stutter in A[str(focus_group)]:
        if check_ultra_conservation(focus_group,spell_stutter-1)==True:
    ...
    fig=plt.figure()
    plt.hist(E,ec="k")
    plt.title("ultra_conservation_value")
    plt.grid()
    fig.savefig("histogram_ultra"+str(focus_group)+".png")
    print(D_2)
    
       
    F=Series(E)
    G=F.describe()
    G.to_csv('site_analysis_'+str(focus_group)+".csv")
